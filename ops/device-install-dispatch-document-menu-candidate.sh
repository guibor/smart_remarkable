#!/bin/bash
# Guarded device-side 11->12 transaction for the Ferrari 3.28.0.169 Dispatch
# notebook/PDF shortcut. It is inert until run by the reviewed Mac controller.
set -Eeuo pipefail
umask 077
...
case "$ACTION" in prepare|activate) ;;
    *) echo "usage: $0 prepare|activate inert|functional STAGE REVIEWED_SHA256 ID" >&2; exit 2 ;;
esac
case "$MODE" in inert|functional) ;;
    *) echo "mode must be inert or functional" >&2; exit 2 ;;
esac
[[ "$ID" =~ ^[0-9]{8}T[0-9]{6}Z-[0-9]+$ ]] || exit 2
[[ "$REVIEWED_MANIFEST_SHA256" =~ ^[0-9a-f]{64}$ ]] || exit 2
case "$STAGE" in
    /home/root/.codex-staging/dispatch-document-menu-*) ;;
...
RECOVERY_BASE=$RECOVERY_PARENT/dispatch-document-menu
RECOVERY=$RECOVERY_BASE/$ID
LOCK=/run/dispatch-document-menu.lock
TIMER_UNIT=dispatch-document-menu-rollback-$ID.timer
SERVICE_UNIT=dispatch-document-menu-rollback-$ID.service
...
verify_stage() {
    ...
    expected=$(printf '%s\n' SHA256SUMS baseline.sha256 device-install.sh functional.qmd inert.qmd panel.qml rollback.sh | sort)
...
"
    ...
    [ "$(wc -l <"$STAGE/SHA256SUMS" | tr -d ' ')" -eq 6 ] || return 1
    listed=$(awk 'NF == 2 { print $2 }' "$STAGE/SHA256SUMS" | sort)
    [ "$listed" = "$(printf '%s\n' baseline.sha256 device-install.sh functional.qmd inert.qmd panel.qml rollback.sh | sort)" ] || return 1
...
verify_stage
mkdir "$LOCK" 2>/dev/null || { echo "another Dispatch document-menu transaction is active" >&2; exit 1; }
...
PRE_STATE=baseline
PRE_PANEL=absent
if [ "$MODE" = functional ]; then PRE_STATE=inert; PRE_PANEL=installed; fi
...
if [ "$MODE" = inert ]; then
    for parent in /home/root/.local /home/root/.local/lib; do
        if [ -e "$parent" ] || [ -L "$parent" ]; then
            [ -d "$parent" ] && [ ! -L "$parent" ] || exit 1
            [ "$(readlink -f "$parent")" = "$parent" ] || exit 1
            [ "$(stat -c %u:%g "$parent")" = 0:0 ] || exit 1
        else
            mkdir "$parent"; chown root:root "$parent"; chmod 0755 "$parent"
        fi
    done
    mkdir "$PANEL_DIR"; chown root:root "$PANEL_DIR"; chmod 0755 "$PANEL_DIR"
    cp "$STAGE/panel.qml" "$RECOVERY/panel.ready"; chown root:root "$RECOVERY/panel.ready"; chmod 0644 "$RECOVERY/panel.ready"
    [ "$(hash_file "$RECOVERY/panel.ready")" = "$EXPECTED_PANEL_SHA256" ]
    mv "$RECOVERY/panel.ready" "$PANEL"
    CANDIDATE=$STAGE/inert.qmd
    EXPECTED_CANDIDATE=$EXPECTED_INERT_SHA256
    POST_STATE=inert
else
...
marker='[qmldiff]: Loading file dispatch-document-menu-3.28.0.169.qmd'
...
if [ "$MODE" = functional ]; then
    grep -Fq '[qmldiff]: Processing file /qml/common/Values.qml...' /tmp/remagic-live-test.log
...
if grep -Ei 'ReferenceError|TypeError|is not a type|Cannot assign|is not installed|QQmlComponent: Component is not ready' /tmp/remagic-live-test.log |
    grep -Ei 'Dispatch shortcut|dispatch-document-menu|DispatchLauncher|SettingsMenu\.qml|MainView\.qml|Values\.qml|remarkable-dispatch-shortcut'; then
    exit 1
fi
...
cleanup_lock
printf 'dispatch_document_menu=%s-committed pid=%s recovery=%s\n' "$MODE" "$pid" "$RECOVERY"
